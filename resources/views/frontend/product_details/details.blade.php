    ...
    @else
        ...
        <div class="mt-3">
            @if ($detailedProduct->digital == 0)
                ...
                @else
                    <button type="button"
                        class="btn btn-warning mr-2 add-to-cart fw-600 min-w-150px rounded-1 text-white"
                        @if (Auth::check()) onclick="addToCart()" @else onclick="showLoginModal()" @endif>
                        <i class="las la-shopping-bag"></i> {{ translate('Add to cart') }}
                    </button>
                    <button type="button" class="btn btn-primary buy-now fw-600 add-to-cart min-w-150px rounded-1"

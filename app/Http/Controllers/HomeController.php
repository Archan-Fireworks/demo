...
class HomeController extends Controller
{
 ...
 public function all_categories_quick(Request $request)
    {
$categoriesWithUniqueProducts = Category::with('products')->where('parent_id', 0)->orderBy('order_level', 'desc')->get();

$uniqueProductIds = collect();
$categories = $categoriesWithUniqueProducts->map(function ($category) use ($uniqueProductIds) {
    $category->products = $category->products->filter(function ($product) use ($uniqueProductIds) {
    ...
    //   return response()->json($categories);
        return view('frontend.quick_purchase', compact('categories'));
    }
    ...
    public function checkPhoneExists(Request $request) {
        ...
        $userExists = User::where('phone', $phone)->where('name','!=','')->exists();
        if(!$userExists) {
            $user = User::create([
                'phone' => $phone,
    ...
    public function loginUser(Request $request) {
        ...
        if($user->verification_code == $request->input('otp')) {
            ...
            return response()->json(['otp' => 'Otp is wrong']);
        }
